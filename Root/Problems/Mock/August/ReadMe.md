...
4. Translate words such as *previous*, *change*, *monthly*, *low*, *high*, *specific*, etc. into SQL operations.
5. Choose the correct SQL technique.
6. Build the query logically.
7. Produce exactly the required output.
...
| Scenario | Metric | Previous Value | Change |
|---|---|---|---|
| Inventory | Closing stock | Previous month's stock | Current − Previous |
| SmartMart | Sales revenue | Previous month's revenue | Current − Previous |
| AdventureWorks | Sales amount | Previous month's sales | Current − Previous |
| Student | Enrollments | Previous month's enrollments | Current − Previous |
...
| previous month | `LAG()` |
| current minus previous | arithmetic difference |
| information from another table | `JOIN` |
| only matching records | `WHERE` |
| top result | `ORDER BY ... DESC` + `LIMIT` |

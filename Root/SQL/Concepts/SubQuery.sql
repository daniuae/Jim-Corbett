...
-- In production SQL, be careful with NOT IN if the subquery can return NULL.
...
-- For date comparisons, use an appropriate date expression rather than treating dates like numeric averages.
-- For example, find customers with orders after a particular derived date:
SELECT DISTINCT
    c.customer_id,
    c.name
FROM customers c
